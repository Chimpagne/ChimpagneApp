...
class NominatimTests {
  ...
  fun testCategoryBuilding() {
    ...
    convertNameToLocations(
      ...
      { locations ->
        ...
        Assert.assertEquals("Wilshire Boulevard, 123, 90210, Beverly Hills, USA", locations.first().name)
  ...
  fun testInvalidJsonResponse() {
    ...
    convertNameToLocations(
      "InvalidJSON",
      { locations ->
        Assert.assertTrue("Expected an empty list of locations due to invalid JSON", locations.isEmpty())
        latch.countDown()
      },
      limit = 5)
