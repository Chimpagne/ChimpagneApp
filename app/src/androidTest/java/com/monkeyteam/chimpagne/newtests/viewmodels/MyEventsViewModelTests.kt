...
class MyEventsViewModelTests {
    ...
    fun fetchingGuestEventsWithCreatedAndJoinedEvents() {
        ...
        assertTrue(eventVM.uiState.value.createdEvents[createdEventForAccount1.id]!!.id == createdEventForAccount1.id)
        assertTrue(eventVM.uiState.value.joinedEvents.size == 1)
        assertTrue(eventVM.uiState.value.joinedEvents[joinedEventForAccount1.id]!!.id == joinedEventForAccount1.id)
    }
